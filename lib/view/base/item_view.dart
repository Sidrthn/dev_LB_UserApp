...
class ItemsView extends StatefulWidget {
  ...
  const ItemsView({super.key, required this.stores, required this.items, required this.isStore, this.isScrollable = false,
    ...
    this.isCampaign = false, this.inStorePage = false, this.isFeatured = false, this.showTheme1Store = false});
      ...
      !isNull ? length > 0 ? GridView.builder(
        ...
        itemBuilder: (context, index) {
          return widget.showTheme1Store ? StoreWidget(store: widget.stores![index], index: index, inStore: widget.inStorePage)
              : ResponsiveHelper.isDesktop(context) && widget.stores != null ? WebStoreWidget(store: widget.stores![index]) :
