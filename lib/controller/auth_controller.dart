...
class AuthController extends GetxController implements GetxService {
    ...
    if (response.statusCode == 200) {
      if (!Get.find<SplashController>().configModel!.customerVerification!) {
        await authRepo.saveUserToken(
            token: response.body["token"], response.body['user']['id']);
  ...
  Future<ResponseModel> login(
    ...
    if (response.statusCode == 200) {
      ...
      responseModel = ResponseModel(true,
          '${response.body['is_phone_verified']}${response.body['phone_verify_end_url']}');
    } else {
      responseModel = ResponseModel(false, response.statusText);
    }
    _isLoading = false;
  ...
  Future<ResponseModel> newUserLogin(
    String? phone,
    String? name,
  ) async {
    _isLoading = true;
    update();
    Response response = await authRepo.newUserLogin(phone: phone, name: name);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true,
          '${response.body['is_phone_verified']}${response.body['phone_verify_end_url']}');
  ...
  Future<void> registerWithSocialMedia(SocialLogInBody socialLogInBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (Get.find<SplashController>().configModel!.customerVerification! &&
          response.body['is_phone_verified'] == 0) {
        Get.toNamed(RouteHelper.getVerificationRoute(
            socialLogInBody.phone, RouteHelper.signUp));
      } else {
        authRepo.saveUserToken(
            token: response.body['token'], response.body['user']['id']);
        await authRepo.updateToken();
  ...
  Future<ResponseModel> verifyEmail(String email, String token) async {
    ...
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      authRepo.saveUserToken(token: token, response.body['user']['id']);
      await authRepo.updateToken();
      Get.find<UserController>().getUserInfo();
      responseModel = ResponseModel(true, response.body["message"]);
    } else {
  ...
  Future<ResponseModel> verifyPhone(
    ...
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      await authRepo.saveUserToken(response.body['user']['id'],
  ...
  void saveUserNumberAndPassword(String number, String countryCode) {
    authRepo.saveUserNumberAndPassword(number, countryCode);
  }
  ...
  String getEarningPint() {
    String value = authRepo.getEarningPint();
    String points = value.split('.')[0];
    return points;
  }
