...
class ConfigController extends Controller
{
    ...
    public function cod_charges(){
        $cod_charges = BusinessSetting::where('key', 'cod_charges')->first();

        return response()->json([
            'charges' => $cod_charges->value,

        ]);
    }

    public function configuration($user_id = null)
    {
        ...
        $cod_prevention = BusinessSetting::where('key', 'cod_prevention')->first();

        if($cod['status'] == null){
            $canceledOrdersCount = 2;
        }elseif($user_id && $cod_prevention->value == 1) {
            $canceledOrdersCount = Order::where('user_id', $user_id)
                ->where('order_status', 'canceled')
                ->count();
        } else {
            $canceledOrdersCount = 0; // Default to 0 if any condition is not met
        }

        $cashOnDeliveryEnabled = $canceledOrdersCount <= 1;
//        dd($canceledOrdersCount);
