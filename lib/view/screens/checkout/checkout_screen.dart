...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initCall() {
    if (Get.find<AuthController>().isLoggedIn()) {
      Get.find<OrderController>().streetNumberController.text =
          Get.find<LocationController>().getUserAddress()!.streetNumber ?? '';
      Get.find<OrderController>().houseController.text =
          Get.find<LocationController>().getUserAddress()!.house ?? '';
      Get.find<OrderController>().floorController.text =
      ...
      if (widget.storeId == null) {
        ...
        Get.find<StoreController>()
            .initCheckoutData2(_cartList![0]!.item!.storeId);
      }
      if (widget.storeId != null) {
      ...
      Get.find<OrderController>().tipController.text =
          Get.find<OrderController>().selectedTips != -1
                  ...
                  for (ZoneData zData in Get.find<LocationController>()
                    ...
                    for (Modules m in zData.modules!) {
                      if (m.id == Get.find<SplashController>().module!.id &&
                          m.pivot!.zoneId == storeController.store!.zoneId) {
                        moduleData = m.pivot;
                        break;
                      }
                    }
                ...
                return GetBuilder<CouponController>(
                  ...
                  return GetBuilder<OrderController>(
                    ...
                    double total = subTotal +
                        ...
                        additionalCharge;
                    // +
                    // orderController.codCharge;
                    total = PriceConverter.toFixed(total);
                    orderController.setTotalAmount(total -
      ...
      child: Column(children: [
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          physics: const BouncingScrollPhysics(),
                          itemCount:
                              storeController.pickedPrescriptions.length + 1,
                          itemBuilder: (context, index) {
                            XFile? file = index ==
                                    storeController.pickedPrescriptions.length
                                ? null
                                : storeController.pickedPrescriptions[index];
                            if (index < 5 &&
                                index ==
                                    storeController
                              ...
                              return InkWell(
                                onTap: () {
                                  if (ResponsiveHelper.isDesktop(context)) {
                                    storeController.pickPrescriptionImage(
        ...
        !takeAway
            ? Center(
                ...
                // Text('${'cod_charge'.tr}: '),
                // Text(
                //   orderController.codCharge.toString(),
                //   textDirection: TextDirection.ltr,
                // ),
              ]))
            : const SizedBox(),
                            ...
                            TextButton.icon(
                              onPressed: () async {
                                var address = await Get.toNamed(
                                    RouteHelper.getAddAddressRoute(true, false,
                                        storeController.store!.zoneId, false));
                                if (address != null) {
                                  ...
                                  orderController.streetNumberController.text =
                                      address.streetNumber ?? '';
                                  orderController.houseController.text =
                                      address.house ?? '';
                                  orderController.floorController.text =
                                      address.floor ?? '';
                                }
                              },
      ...
      child: Column(children: [
        Container(
          ...
          child: Column(children: [
            ...
            PartialPayView(
                totalPrice: total, isPrescription: widget.storeId != null),
          ]),
        ),
        const SizedBox(height: Dimensions.paddingSizeSmall),
        Container(
          ...
          child:
            ...
            NoteAndPrescriptionSection(
                orderController: orderController, storeId: widget.storeId),
            ...
            orderController.paymentMethodIndex == 0 &&
                    Get.find<OrderController>().isEnableCOD
                ? Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                        Text('cod_charge'.tr, style: robotoRegular),
                        Text(
                          '(+) ${PriceConverter.convertPrice(orderController.codCharge)}',
                          style: robotoRegular,
                          textDirection: TextDirection.ltr,
                        ),
                      ])
            ...
            (ResponsiveHelper.isDesktop(context) ||
                ...
                : const SizedBox(),
            SizedBox(
                height: orderController.isPartialPay
                    ? Dimensions.paddingSizeSmall
                    : 0),
  ...
  void _callback(bool isSuccess, String? message, String orderID, int? zoneID,
    ...
    if (isSuccess) {
      if (widget.fromCart && orderController.paymentMethodIndex != 2) {
      ...
      if (Get.find<OrderController>().paymentMethodIndex == 2) {
        if (GetPlatform.isWeb) {
          ...
          String selectedUrl;
          selectedUrl =
              '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<UserController>().userInfoModel!.id}&payment_method=${Get.find<OrderController>().digitalPaymentName}&payment_platform=web&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
    ...
    return Container(
      ...
      child: SafeArea(
        child: CustomButton(
            ...
            buttonText: orderController.isPartialPay
                ? 'place_order'.tr
                : 'confirm_order'.tr,
            onPressed: orderController.acceptTerms
                ? () {
                    ...
                    if (!_isCashOnDeliveryActive! &&
                    ...
                    } else if (orderController.tipController.text.isNotEmpty &&
                        orderController.tipController.text != 'not_now' &&
                        double.parse(
                                orderController.tipController.text.trim()) <
                            0) {
                      ...
                      if (widget.storeId == null) {
                            ...
                            PlaceOrderBody(
                              ...
                              contactPersonNumber:
                                  finalAddress.contactPersonNumber ??
                                      Get.find<UserController>()
                                          .userInfoModel!
                                          .phone,
                              streetNumber: orderController
                                  .streetNumberController.text
                                  .trim(),
                              house:
                                  orderController.houseController.text.trim(),
                              floor:
                                  orderController.floorController.text.trim(),
                              discountAmount: discount,
                              taxAmount: tax,
                              receiverDetails: null,
                              parcelCategoryId: null,
                              chargePayer: null,
                              dmTips: (orderController.orderType ==
                                          'take_away' ||
                                      orderController.tipController.text ==
                                          'not_now')
                                  ? ''
                                  : orderController.tipController.text.trim(),
                              cutlery:
                              ...
                              deliveryInstruction:
                                  orderController.selectedInstruction != -1
                                      ...
                                      : '',
                              partialPayment:
                                  orderController.isPartialPay ? 1 : 0,
                              codAmount: orderController.codCharge,
                            ),
                        ...
                        orderController.placePrescriptionOrder(
                            ...
                            orderController.noteController.text,
