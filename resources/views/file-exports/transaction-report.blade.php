            ...
            <thead>
                ...
                <tr>
                    ...
                    <th>
                        ...
                        <br>
                        {{ translate('Delivery_Man_Earnings') }} -
                        {{ $data['deliveryman_earned'] ?? translate('N/A') }}
                    </th>
                    <th> </th>
                    <th></th>
                    <th></th>
            ...
            <tbody>
                @foreach ($data['order_transactions'] as $key => $ot)
                    <tr>
                        ...
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->order ? $ot->order['order_amount'] - $ot->order['dm_tips'] - $ot->order['delivery_charge'] - $ot['tax'] + $ot->order['coupon_discount_amount'] + $ot->order['store_discount_amount'] : 0) }}
                        </td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->order ? $ot->order->details->sum('discount_on_item') : 0) }}
                        </td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->order['coupon_discount_amount'] ?? 0) }}
                        </td>
                        <td>{{ \App\CentralLogics\Helpers::number_format_short($ot->order ? $ot->order['coupon_discount_amount'] + $ot->order['store_discount_amount'] : 0) }}
                        </td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->tax ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->delivery_charge ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->order_amount ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->admin_expense ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->discount_amount_by_store ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->order ? $ot->admin_commission + $ot->admin_expense - $ot->delivery_fee_comission : 0) }}
                        </td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->additional_charge ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->delivery_fee_comission ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->admin_commission ?? 0) }}</td>
                        <td>{{ \App\CentralLogics\Helpers::format_currency($ot->store_amount ? $ot->store_amount - $ot->tax : 0) }}
                        </td>
                        ...
                        <td class="mw--85px text-capitalize min-w-120">
                            @if ($ot->order && isset($ot->order['payment_method']))
