  ...
  void splitPhoneNumber(String number) async {
    if (GetPlatform.isAndroid || GetPlatform.isIOS) {
      try {
        ...
        // _contactPersonNumberController.text = phoneNumber.international;
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
          title: widget.address == null
              ? 'add_new_address'.tr
              : 'update_address'.tr,
          backButton: widget.fromAuth ? false : true),
      body: SafeArea(
        child: isLoggedIn
            ? GetBuilder<UserController>(builder: (userController) {
                if (widget.address != null) {
                  if (widget.address!.contactPersonNumber!
                      .startsWith(_countryDialCode!)) {
                    widget.address!.contactPersonNumber = widget
                        .address!.contactPersonNumber!
                        .replaceFirst(_countryDialCode!, '');
                  }
                  splitPhoneNumber(widget.address!.contactPersonNumber!);
                  _contactPersonNameController.text =
                      widget.address!.contactPersonName ?? '';
                  _streetNumberController.text =
                      widget.address!.streetNumber ?? '';
  ...
  Widget button(LocationController locationController) {
    return Container(
      ...
      child: CustomButton(
        ...
        isLoading: locationController.isLoading,
        buttonText:
            widget.address == null ? 'save_location'.tr : 'update_address'.tr,
            ...
            : () async {
                ...
                if (_addressController.text.isEmpty) {
                  ...
                  AddressModel addressModel = AddressModel(
                    ...
                    longitude: locationController.position.longitude.toString(),
                    zoneId: locationController.zoneID,
                    zoneIds: [locationController.zoneID],
                    streetNumber: _streetNumberController.text,
                    house: _houseController.text,
                    floor: _floorController.text,
                  );
                  ...
                  if (widget.address == null) {
                    locationController
                        .addAddress(addressModel, widget.fromAuth,
                            widget.fromCheckout, widget.fromRide, widget.zoneId)
                        .then((response) {
                      if (response.isSuccess) {
