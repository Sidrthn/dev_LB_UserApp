    ...
    if (isSuccess) {
      ...
      if (Get.find<ParcelController>().paymentIndex == 2) {
        if (GetPlatform.isWeb) {
          ...
          String selectedUrl =
              '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<UserController>().userInfoModel!.id}&payment_method=${Get.find<ParcelController>().digitalPaymentName}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
              ...
              if (parcelController.distance == -1) {
                ...
                Get.find<OrderController>().placeOrder(
                    PlaceOrderBody(
                      ...
                      deliveryInstruction: '',
                      partialPayment: 0,
                      codAmount: Get.find<OrderController>().codCharge,
                    ),
