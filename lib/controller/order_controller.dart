...
class OrderController extends GetxController implements GetxService {
  ...
  bool get isEnableCOD => _isEnableCOD;
  ...
  double get codCharge => _codCharge;
  ...
  Future getCODCharge(int index) async {
    _codCharge = 0.0;
    if (index == 0) {
      Response response = await orderRepo.getCODCharge();
      if (response.statusCode == 200) {
        Map<String, dynamic> responseBody = response.body;
        _codCharge = double.parse(responseBody['charges']);
      } else {
        _codCharge = 0.0;
      }
    }
  ...
  Future checkEnableCOD() async {
    int isCodEnable = 1;
    var userId = Get.find<UserController>().userInfoModel!.id;
    Response response = await orderRepo.checkEnableCOD("$userId");
    if (response.statusCode == 200) {
      Map<String, dynamic> responseBody = response.body;
      isCodEnable = responseBody['cod'];
      if (isCodEnable == 1) {
        _isEnableCOD = true;
      } else {
        _isEnableCOD = false;
